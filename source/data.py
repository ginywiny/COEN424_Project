...
def get_download_dir():
    default_dir = os.path.join('/tmp/dataset', '.dgl')
...
class MovieLens(object):
    ...
    def _load_raw_user_info(self):
        ...
        if self._name == 'ml-100k':
            ...
            self.user_info = pd.read_csv(os.path.join(self._dir, 'u.user'), sep='|', header=None, encoding = "ISO-8859-1", 
            
            # self.user_info = pd.read_csv(os.path.join(self._dir, 'u.user'), sep='|', header=None, encoding = "ISO-8859-1"
                                    names=['id', 'age', 'gender', 'occupation', 'zip_code'], engine='python')
        
        elif self._name == 'ml-1m':
            self.user_info = pd.read_csv(os.path.join(self._dir, 'users.dat'), sep='::', header=None, encoding = "ISO-8859-1",
                                    names=['id', 'gender', 'age', 'occupation', 'zip_code'], engine='python')
        elif self._name == 'ml-10m':
            rating_info = pd.read_csv(
                os.path.join(self._dir, 'ratings.dat'), sep='::', header=None, encoding = "ISO-8859-1",
                names=['user_id', 'movie_id', 'rating', 'timestamp'],
