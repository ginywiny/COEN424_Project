...
def infer(user_ratings, top_n):
    ...
    net = net.to('cpu')
    net.load_state_dict(th.load("./model/model.pth", map_location=th.device('cpu')))
    net.eval()
    ...
    user_rating_movies = list()
    for key in user_ratings.keys():
        user_rating_movies.append(int(key))
    ...
    # indices = th.topk(real_pred_ratings.flatten(), top_n + 10).indices
    indices = th.topk(real_pred_ratings.flatten(), top_n + 20).indices
    indices = [int(i) for i in indices if int(i)+1 not in user_rating_movies]
    movie_names = []
    for indice in indices[0:top_n]:
